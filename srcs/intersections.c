...
/*   Updated: 2016/06/30 19:41:50 by gpinchon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
enum e_bool	intersect_cylinder(t_primitive cp, t_ray r, float *current_z)
{
	...
	f[0] = vec3_dot(r.direction, r.direction) - pow(vec3_dot(r.direction, cp.direction), 2);
	f[1] = 2.0 * (vec3_dot(r.direction, eye) - vec3_dot(r.direction, cp.direction) * vec3_dot(eye, cp.direction));
	f[2] = vec3_dot(eye, eye) - pow(vec3_dot(eye, cp.direction), 2) - cp.radius2;
