...
void	*new_image(t_framework *framework, int width, int height, char *name)
{
	...
	if (!framework || !framework->mlx_ptr || !(img = ft_memalloc(sizeof(t_img))))
		return (NULL);
	img->mlx_img = mlx_new_image(framework->mlx_ptr, width, height);
	img->data = mlx_get_data_addr(img->mlx_img, &img->bpp, &img->size_line, &img->endian);
	img->size = (t_point2){width, height};
	img->opp = img->bpp / 8;
	img->mlx_ptr = framework->mlx_ptr;
	img->name = name;
	return (add_image(framework, clear_image(img)));
}
